<template>
  <nav aria-label="Progress">
    <ol
      role="list"
      class="divide-y divide-gray-300 rounded-md border border-white md:flex md:divide-y-0"
    >
      <li
        v-for="step in steps"
        :key="step.name"
      ...
      >
        <a
          v-if="step.status === EStepStatus.complete"
          class="group flex w-full items-center"
        >
          <span class="flex items-center px-6 py-4 text-sm font-medium">
            <span
              class="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full bg-slate-600"
            >
        ...
        <a
          v-else-if="step.status === EStepStatus.current"
          class="flex items-center px-6 py-4 text-sm font-medium"
          aria-current="step"
        >
          <span
            class="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-full border-2 border-pink-300"
          >
            <span class="text-pink-300">{{ step.id }}</span>
          </span>
          <span class="ml-4 text-sm font-medium text-pink-300">{{
            $t(step.name)
          }}</span>
        </a>
        <a
          v-else-if="step.status === EStepStatus.empty"
