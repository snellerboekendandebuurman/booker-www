    ...
    <div class="md:col-span-2 z-15 mt-1">
      <div class="relative inline-block w-full">
        <button @click="toggleIsOpen" type="button"
          :class="[membershipIdValue ? 'cursor-pointer' : 'cursor-auto', ,'w-full bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500']">
          <div v-if="sportsTypeValue" class="flex items-center cursor-pointer">
            <span class="ml-4">{{ sportsTypeValue }}</span>
          </div>
          <div v-else-if="membershipIdValue" class="flex items-center cursor-pointer">
            <span class="ml-4">{{ $t("global.fields.select_sports_type") }}</span>
          </div>
          <div v-else>
        ...
        <div v-if="isOpen && membershipIdValue"
          class="absolute z-20 mt-2 w-full bg-white shadow-lg rounded-lg border border-gray-300 divide-y divide-gray-300">
          <div v-if="membershipIdValue" v-for="sportsType in reservationsService.sportsTypes" :key="sportsType" @click="selectSportsType(sportsType)"
