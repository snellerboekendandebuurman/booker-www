<template>
    <div v-if="membershipIdIdValue">
      <label for="player_1_id" class="block text-sm font-medium leading-6 text-slate-700 dark:text-slate-300">{{ $t("global.fields.select_player_1") }}</label>
      <div class="relativemt-1 mt-1">
        ...
        <div class="flex justify-between items-center relative">
          ...
          <img v-if="reservationsService.selectedPlayer1" :src="reservationsService.selectedPlayer1.image" class="rounded-md h-8 w-auto absolute right-0 mr-6" />
        </div>
        ...
        <ErrorMessage name="player_1_id" as="p" class="dark:text-pink-200 text-pink-400" />
      </div>
    </div>
  </template>
