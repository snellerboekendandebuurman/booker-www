<template>
    <div v-if="membershipIdIdValue">
      <label for="player_3_id" class="block text-sm font-medium leading-6 text-slate-700 dark:text-slate-300">{{ $t("global.fields.select_player_3") }}</label>
      <div class="relativemt-1 mt-1">
        <Observer>
        <div class="flex justify-between items-center relative">
          <Field name="player_3_id" class="hidden" />
          <Field name="player_3_full_name" class="hidden" />
          <input
            ref="inputRef"
            ...
            'pr-16': reservationsService.selectedPlayer3?.image,
            'pr-2': !reservationsService.selectedPlayer3?.image
          }" autocomplete="off" type="text" v-model="selectedPlayerFullName" />
          <img v-if="reservationsService.selectedPlayer3" :src="reservationsService.selectedPlayer3.image" class="rounded-md h-8 w-auto absolute right-0 mr-6" />
        </div>
        ...
        <ErrorMessage name="player_3_id" as="p" class="dark:text-pink-200 text-pink-400" />
      </div>
    </div>
  </template>
  ...
  const { value: playerIdValue } = useField("player_3_id");
  const { value: playerFullNameValue } = useField("player_3_full_name");
  ...
  onBeforeMount(() => {
   if (reservationsService.selectedPlayer3) {
    isPlayerSelected.value = true;
    selectedPlayerFullName.value = reservationsService.selectedPlayer3.fullName
    ;
    playerIdValue.value = reservationsService.selectedPlayer3.id;
    playerFullNameValue.value = reservationsService.selectedPlayer3.fullName;
   }
  });
  ...
  function selectPlayer(player: Player) {
    ...
    reservationsService.selectedPlayer3 = player;
