<template>
  ...
  >
    <label
      ...
      >{{ $t("global.fields.select_club") }}</label
    >
    <div class="relativemt-1 mt-1">
      <Observer>
        <div class="flex justify-between items-center relative">
          ...
          />
          <img
            v-if="membershipsService.selectedClub"
            :src="membershipsService.selectedClub.image"
            class="rounded-md h-8 w-auto absolute right-0 mr-6"
          />
        </div>
        <div
          v-if="membershipsService.clubs.length !== 0"
        ...
        >
          <div
            v-for="club in membershipsService.clubs"
            :key="club.id"
...
function shouldDisplayField(clientId: string | number, name: string): boolean {
  return membershipsService.getClient(clientId).name.includes(name)!;
}
