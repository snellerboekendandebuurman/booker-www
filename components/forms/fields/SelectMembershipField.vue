<template>
  <div>
    <label
      ...
      >{{ $t("global.fields.membership") }}</label
    >
    <Field name="membership_id" class="hidden" />
    <div class="md:col-span-2 mt-1">
      <div class="relative inline-block w-full">
        ...
        >
          <div
            v-if="membershipIdIdValue"
            class="flex justify-between items-center cursor-pointer"
          >
            <span class="ml-4">{{
              membershipsService.getMembership(membershipIdIdValue as number)
                .associationNumber ||
              membershipsService.getMembership(membershipIdIdValue as number)
                .clubNumber
            }}</span>
