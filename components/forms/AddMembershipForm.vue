<template>
    <div>
      ...
      <FormStep>
        <SelectClientField />
        <SelectAuthenticationMethodField @authentication-method-selected="handleAuthenticationMethodSelected"/>
      </FormStep>
...
const selectedAuthenticationMethod: Ref<null | AuthenticationMethod> = ref(null);
...
const validationSchema = [
  object({
    ...
    authentication_method_id: number().required(t("global.messages.field_authentication_method_id_required")),
  }),
  object({
    club_id: string()
      .when('client_id', (_, schema) =>
      shouldDisplayFieldBasedOnClient(membershipsService.selectedClient?.id || 0, "kntlb")
    ...
    club_image: string(),
    club_number: number()
      .when('authentication_method_id', (_, schema) =>
    ...
    association_number: number()
      .when('authentication_method_id', (_, schema) =>
        shouldDisplayField("association")
            ? schema.required(t("global.messages.field_association_number_required")) 
            : schema
      ),
    password: string()
    .when('authentication_method_id', (_, schema) =>
...
function handleAuthenticationMethodSelected(authenticationMethod: AuthenticationMethod) {
  selectedAuthenticationMethod.value = authenticationMethod
}

function shouldDisplayField(name: string): boolean {
  return selectedAuthenticationMethod.value?.method.includes(name)!
}

function shouldDisplayFieldBasedOnClient(client_id: string | number, name: string): boolean {
  return membershipsService.getClient(client_id)?.name.includes(name) || false;
}
