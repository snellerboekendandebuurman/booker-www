<template>
  <div class="m-4">
    <ReservationAddModal :modal-open="modalAddOpen" @toggle-modal="toggleAddModal"/>
    <ul role="list" class="grid grid-cols-1 gap-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
    <ReservationAddCard @toggle-modal="toggleAddModal"/>
    <Observer>
        <ReservationInfoCard v-for="reservation in reservationsService.reservations" :key="reservation.id" :reservation="reservation"/>
    </Observer>
  </ul>
  </div>
</template>
