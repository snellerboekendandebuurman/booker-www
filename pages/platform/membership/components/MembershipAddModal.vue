<template>
        ...
        <div class="flex min-h-full items-stretch justify-center text-center md:items-center md:px-2 lg:px-4">
          <TransitionChild as="template" enter="ease-out duration-300" enter-from="opacity-0 translate-y-4 md:translate-y-0 md:scale-95" enter-to="opacity-100 translate-y-0 md:scale-100" leave="ease-in duration-200" leave-from="opacity-100 translate-y-0 md:scale-100" leave-to="opacity-0 translate-y-4 md:translate-y-0 md:scale-95">
            <DialogPanel class="flex w-full transform text-left text-base transition md:my-8 md:max-w-2xl md:px-4 lg:max-w-4xl">
              <div class="relative w-full rounded items-center sm:overflow-hidden md:overflow-visible px-4 pb-8 pt-14 shadow-2xl sm:px-6 sm:pt-8 md:p-6 lg:p-8 bg-slate-300 dark:bg-slate-900">
                  <button type="button" class="absolute right-4 top-4 hover:text-gray-500 sm:right-6 sm:top-8 md:right-6 md:top-6 lg:right-8 lg:top-8" @click="closeStep">
                    <span class="sr-only">Close</span>
                    <XMarkIcon class="h-6 w-6 dark:text-slate-300" aria-hidden="true" />
                  </button>
                  <FormHeaderSteps class="mt-8 md:mt-12 my-4" :steps="addMembershipSteps" />
                  <AddMembershipForm @next-step="nextStep" @previous-step="previousStep" @close-step="closeStep" class="mt-8"/>
              </div>
            </DialogPanel>
          </TransitionChild>
        </div>
  ...
  import AddMembershipForm from "~/components/forms/AddMembershipForm.vue"
  ...
  await onBeforeMount(async () => {
    const response = await membershipsService.getClients({
    ...
    membershipsService.setClients(response.data.value);
  });

  let addMembershipSteps = reactive([
    { id: 1, name: 'platform.memberships.step_select_membership', status: EStepStatus.current},
    { id: 2, name: 'platform.memberships.step_add_credentials', status: EStepStatus.empty},
  ])
